<script lang="ts">
	...
	/**
	 * Checks if a topic matches the search
...
<div
	class={'w-full md:w-6/12 xl:w-4/12 h-full md:px-5 flex flex-col md:mt-0 mt-10 ' +
...
>
	...
	<div class="overflow-y-scroll scrollbar">
		{#if !topics}
			<p>Loading topics...</p>
		{:else if topics.some((t) => doesTopicMatchSearch(t, search))}
			{#each topics as topic}
				{#if doesTopicMatchSearch(topic, search)}
					<div
