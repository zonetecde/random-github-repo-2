...
>
	...
	>
		...
		{#if randomRepo.ProgrammingLanguage.trim() !== ''}
			<span
				class="ml-auto max-w-[130px] h-[80%] md:h-[63%] bg-[#212831] py-2 px-4 rounded-xl shadow-sm shadow-black flex justify-center"
			>
				<span
		...
		<span
			class={'w-32 bg-[#212831] py-2 px-3  h-[80%] md:h-[63%] rounded-xl shadow-sm shadow-black flex justify-center ' +
				(randomRepo.ProgrammingLanguage.trim() === '' ? 'ml-auto' : '')}
		>
			<svg class="w-5 mt-0.5" viewBox="0 0 16 16" version="1.1" fill="#b8b8b0" aria-hidden="true"
				><path
